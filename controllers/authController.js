...
const { promisify } = require('util'); // = const util = require('util');
...
const createSendToken = (user, statusCode, res) => {
  ...
  // remove the password from output in Postman:
...
exports.login = catchAsync(async (req, res, next) => {
  ...
  const { email, password } = req.body; // pareil que : const email = req.body.email; et const password = req.body.password
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if(currentUser.changedPasswordAfter(decoded.iat)) {
    return next(new AppError('User recently changer password', 401));
  }
